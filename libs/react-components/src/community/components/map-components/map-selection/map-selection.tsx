...
import styles from './map-selection.module.scss';

type MapSelectionType = 'button' | 'historical' | 'selection';
...
export const MapSelection = (props: MapSelectionProps): JSX.Element => {
  ...
  const mapSelectionBEM = classNames(
    styles['tedi-map-selection__wrapper'],
    selected && styles['tedi-map-selection--selected'],
    type && styles[`tedi-map-selection--${type}`],
    multiple && styles['tedi-map-selection--multiple'],
    className
  );

  return (
    <div role="button" tabIndex={0} onClick={onSelect} onKeyDown={handleKeyDown} className={mapSelectionBEM} id={id}>
      <div className={styles['tedi-map-selection__content']}>{content}</div>
      <div className={styles['tedi-map-selection__title']}>{title}</div>
    </div>
  );
};
