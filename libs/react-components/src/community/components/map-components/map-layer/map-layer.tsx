...
const MapLayer = (props: MapLayerProps): JSX.Element => {
  ...
  const renderItem = (item: LayerOption, level = 0) => {
    ...
    return (
      <div
        key={item.id}
        className={classNames(styles['tedi-map-layer__item'], styles[`tedi-map-layer__level-${level}`])}
      >
        <div className={styles['tedi-map-layer__row']}>
          {item.type === 'checkbox' && (
            <Checkbox
              id={item.id}
              label={item.label}
          ...
          {item.type === 'checkbox' && (
            <div className={styles['tedi-map-layer__actions']}>
        ...
        {item.type === 'label' && (
          <div key={item.id} className={styles['tedi-map-layer__item']}>
            <div className={styles['tedi-map-layer__label']}>{item.label}</div>
  ...
  return (
    <div className={styles['tedi-map-layer']}>
      <div className={styles['tedi-map-layer__select-all']}>
        {hasCheckboxes(items) ? (
          <Checkbox
            id="select-all"
      ...
      <div
        className={classNames(
          styles['tedi-map-layer'],
          !hasCheckboxes(items) && styles['tedi-map-layer--no-checkboxes']
        )}
      >
