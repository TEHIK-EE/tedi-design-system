...
@Component({
  ...
  host: {
    "[class]": "classes()",
    "[style.width]": "axis() === 'horizontal' ? size() : '0px'",
    "[style.height]": "axis() === 'vertical' ? size() : '0px'",
  }
})
export class SeparatorComponent {
  ...
  classes = computed(() => {
    const classList = ['tedi-separator', `tedi-separator--${this.color()}`, `tedi-separator--${this.axis()}`];
